...
def load_history(user_id: str = "USR-00000000", limit: int = None) -> list:
    """Load last N messages for a specific user from SQLite."""
    if limit is None:
        limit = HISTORY_LIMIT
    return database.get_history("marin", limit=limit, user_id=user_id)


def save_to_history(user_id: str, user_msg: str, marin_reply: str):
    """Save one exchange to SQLite scoped by user_id."""
    database.save_message("marin", "user", user_msg, user_id=user_id)
    database.save_message("marin", "assistant", marin_reply, user_id=user_id)
...
async def main(prompt: str, image_path: str = None, user: str = OWNER_USER, game_context: str = None):
    ...
    user_id = user.get("user_id", "USR-00000000") if isinstance(user, dict) else user
    history = load_history(user_id=user_id, limit=20)
    ...
    try:
        ...
        save_to_history(user_id, bare_question, full_response)
        save_vibe(user_vibe, marin_vibe)
