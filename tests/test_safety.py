...
def test_hitl_confirmation():
    ...
    assert not agent_needs_confirmation("marin", "terminal_tool")
    # system agent has restart_service
    assert agent_needs_confirmation("system", "restart_service")
    assert not agent_needs_confirmation("system", "weather_tool")
